...
DrawCanvas.downScale = function (grayMatrix, scale) {
    // Takes a matrix of gray values and calculates the sum of the included pixels.

    var smallMatrix = new Array();

    for (var i = 0; i < grayMatrix.length; i+= scale) {
        var row = new Array();
        for (var j = 0; j < grayMatrix[i].length; j+= scale) {
            var tmp = 0;
